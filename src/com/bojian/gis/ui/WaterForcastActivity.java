...
import com.bojian.gis.adapter.WaterForcastAdapter;
...
public class WaterForcastActivity extends BaseActivity implements
		OnClickListener {
	private static final String TAG = "WaterForcastActivity";
	private Button backBtn;
	private TextView titleTv;
	private Button otherBtn;
	private WaterForcast mWaterForcast;
	...
	private WaterForcastAdapter adapter;
	private List<WaterForcastBean> Mlist;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.water_forcast);
		init();
		init_titlebar();
		startLoad();
	...
	private void init() {
		// TODO Auto-generated method stub
		sp = new SharePreUtil(WaterForcastActivity.this);
		username = sp.getStringValueByKey("username");
		password = sp.getStringValueByKey("password");
		listView = (ListView) findViewById(R.id.water_forcast_listview);
	}

	private void startLoad() {
		// TODO Auto-generated method stub
		mWaterForcast = new WaterForcast(username, password, startdate,
				enddate, rvsectId);
		ss = new SyncService(WaterForcastActivity.this, mWaterForcast);
	...
	private void init_titlebar() {
		...
		titleTv.setText(R.string.flood_forcast);
	...
	private void startIntent() {
		...
		intent.setClass(WaterForcastActivity.this, TimeQueryActivity.class);
	...
	public void onDataRequestSuccess() {
		...
		Mlist = mWaterForcast.getList();
		if (Mlist != null && Mlist.size() > 0) {
			new Thread(update).run();
		} else {
			Toast.makeText(WaterForcastActivity.this, "无数据！",
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == 2) {
			...
			mWaterForcast = new WaterForcast(username, password, startdate, enddate, rvsectId);
			sNew = new SyncService(WaterForcastActivity.this, mWaterForcast);
	...
	Handler updateHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			adapter = new WaterForcastAdapter(WaterForcastActivity.this, Mlist);
			if (UrlLib.mvalue == null || UrlLib.mvalue.size() == 0) {
				for (int i = 0; i < Mlist.size(); i++) {
					UrlLib.mvalue.add(Mlist.get(i).getSitename());
					UrlLib.mkey.add(Mlist.get(i).getRvsectId());
				}
			}
